...
export async function handler(): Promise<void> {
    ...
    const mapEventsOutput: MapEventsData = {
      _readme: {
        description:
          'Map events schedule for shiesty raiders generated by scheduled updater',
        format:
      ...
      metadata: {
        ...
        ignoredMapNames: [...ignoredMapNames].sort((a, b) =>
          a.localeCompare(b),
        ),
        ignoredEntriesCount: ignoredEntries.length,
        unknownEventTypeIds: [...unknownEventTypeIds].sort((a, b) =>
          a.localeCompare(b),
        ),
      },
    ...
    try {
        const response = await fetch(url, {
          headers: {
            ...
            'user-agent': 'shiesty-schedule-updater/1.0',
          },
