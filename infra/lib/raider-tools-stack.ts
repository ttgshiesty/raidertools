/**
 * SHiESTY RAiDERS Infrastructure Stack
 *
 * Single us-east-2 stack that owns everything behind
 * `api.shiesty.me` and the `auth.shiesty.me` Cognito domain:
...
export interface RaiderToolsStackProps extends cdk.StackProps {
  ...
  /**
   * ARN of the us-east-1 ACM cert for `authDomainName`.
   */
...
export class RaiderToolsStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: RaiderToolsStackProps) {
    ...
    if (cdk.Stack.of(this).region !== 'us-east-2') {
      throw new Error('This stack is intended for us-east-2.');
    }
    ...
    // -----------------------------------------------------------------
    const discordSecret = new secretsmanager.Secret(
      this,
      'DiscordOAuthSecret',
      {
        secretName: props.discordSecretName,
        description:
          'Discord OAuth client credentials + HMAC state-signing key',
        secretObjectValue: {
    ...
    const embarkOauthSecret = new secretsmanager.Secret(
      this,
      'EmbarkOAuthSecret',
      {
    ...
    // -----------------------------------------------------------------
    const defineAuthFn = this.makeLambda(
      'DefineAuthFn',
      'cognito-define-auth.ts',
      {
        timeout: cdk.Duration.seconds(5),
        memorySize: 128,
      },
    );
    const createAuthFn = this.makeLambda(
      'CreateAuthFn',
    ...
    const verifyAuthFn = this.makeLambda(
      'VerifyAuthFn',
    ...
    // production-grade deliverability from `noreply@shiesty.me`.
    // -----------------------------------------------------------------
    this.userPool = new cognito.UserPool(this, 'UserPool', {
      userPoolName: 'shiesty-users',
      ...
      userVerification: {
        emailSubject: 'Verify your SHiESTY RAiDERS account',
        ...
        emailBody: [
          ...
          '  <p>Welcome to <strong>SHiESTY RAiDERS</strong>!</p>',
          ...
          '  <p style="color:#555;">The code expires in 24 hours. Once confirmed, you can sign in at <a href="https://shiesty.me">shiesty.me</a>.</p>',
          '  <p style="color:#888;font-size:12px;margin-top:24px;">If you didn\'t sign up for SHiESTY RAiDERS, you can safely ignore this email — no account will be created.</p>',
          '</div>',
        ].join('\n'),
    ...
    new cognito.CfnUserPoolGroup(this, 'EmbarkAuthGroup', {
      ...
      description:
        'Enables Embark account linking and Embark-backed SHiESTY RAiDERS features.',
    });
    ...
    this.userPoolClient = this.userPool.addClient('SpaClient', {
      userPoolClientName: 'shiesty-spa',
      authFlows: {
        userSrp: true,
        custom: true,
      },
    ...
    // Schedule Lambdas.
    // -----------------------------------------------------------------
    const scheduleReadFn = new nodeLambda.NodejsFunction(
      this,
      'ScheduleReadFunction',
      {
    ...
    const scheduleUpdateFn = new nodeLambda.NodejsFunction(
      this,
      'ScheduleUpdateFunction',
      {
        ...
        environment: {
          ...
          EVENT_TYPES_URL: 'https://shiesty.me/data/schedule/event-types.json',
          SCHEDULE_BUCKET_NAME: scheduleBucket.bucketName,
          SCHEDULE_KEY: 'map-events.json',
    ...
    new events.Rule(this, 'ScheduleUpdaterHourlyRule', {
      description:
        'Refresh ARC Raiders map schedule from map-conditions HTML every hour',
    ...
    const arctrackerUserProxyFn = this.makeLambda(
      'ArctrackerUserProxyFn',
      'arctracker-user-proxy.ts',
      {
        timeout: cdk.Duration.seconds(10),
        memorySize: 256,
        environment: {
    ...
    const metaforgeStatsFn = this.makeLambda(
      'MetaForgeStatsFn',
      'metaforge-stats.ts',
      {
    ...
    const embarkInventoryFn = this.makeLambda(
      'EmbarkInventoryFn',
      'embark-inventory.ts',
      {
        timeout: cdk.Duration.seconds(20),
    ...
    embarkInventoryFn.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          'ssm:DescribeParameters',
          'ssm:GetParameter',
    ...
    const embarkQuestsFn = this.makeLambda(
      'EmbarkQuestsFn',
      'embark-quests.ts',
      {
        timeout: cdk.Duration.seconds(20),
    ...
    embarkQuestsFn.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          'ssm:DescribeParameters',
          'ssm:GetParameter',
    ...
    const embarkProjectsFn = this.makeLambda(
      'EmbarkProjectsFn',
      'embark-projects.ts',
      {
        timeout: cdk.Duration.seconds(20),
    ...
    embarkProjectsFn.addToRolePolicy(
      new iam.PolicyStatement({
        actions: [
          'ssm:DescribeParameters',
          'ssm:GetParameter',
    ...
    const arctrackerUserProxyIntegration =
      new integrations.HttpLambdaIntegration(
        'ArctrackerUserProxyIntegration',
        arctrackerUserProxyFn,
      );
    this.httpApi.addRoutes({
    ...
    const metaforgeStatsIntegration = new integrations.HttpLambdaIntegration(
      'MetaForgeStatsIntegration',
      metaforgeStatsFn,
    );
    this.httpApi.addRoutes({
      path: '/me/metaforge/stats',
      methods: [apigwv2.HttpMethod.GET],
      integration: metaforgeStatsIntegration,
      authorizer: jwtAuthorizer,
    });
    ...
    new cdk.CfnOutput(this, 'EmbarkSnapshotBucketName', {
      value: embarkSnapshotBucket.bucketName,
    });
    new cdk.CfnOutput(this, 'DiscordCallbackUrl', {
      value: `${apiOrigin}/auth/discord/callback`,
    });
    new cdk.CfnOutput(this, 'EmbarkOAuthSecretArn', {
      value: embarkOauthSecret.secretArn,
    });
    new cdk.CfnOutput(this, 'EmbarkManifestParamName', {
      value: embarkManifestParamName,
    });
    new cdk.CfnOutput(this, 'EmbarkUserAgentParamName', {
      value: embarkUserAgentParamName,
    });
    new cdk.CfnOutput(this, 'ScheduleMapEventsUrl', {
      value: `${apiOrigin}/schedule/map-events.json`,
    });
    new cdk.CfnOutput(this, 'ScheduleHealthUrl', {
      value: `${apiOrigin}/schedule/health.json`,
    });
  }
